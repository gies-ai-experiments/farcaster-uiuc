...
run_worker() {
    ...
    docker compose exec shuttle sh -c "cd /app/packages/shuttle && POSTGRES_URL=$POSTGRES_URL REDIS_URL=$REDIS_URL HUB_HOST=$HUB_HOST HUB_SSL=$HUB_SSL yarn start worker"
}
...
run_backfill() {
    ...
    docker compose exec shuttle sh -c "cd /app/packages/shuttle && POSTGRES_URL=$POSTGRES_URL REDIS_URL=$REDIS_URL HUB_HOST=$HUB_HOST HUB_SSL=$HUB_SSL MAX_FID=100 yarn start backfill"
}

# Function to start shuttle sync
run_sync() {
    echo "Starting shuttle sync from event stream..."
    docker compose exec shuttle sh -c "cd /app/packages/shuttle && POSTGRES_URL=$POSTGRES_URL REDIS_URL=$REDIS_URL HUB_HOST=$HUB_HOST HUB_SSL=$HUB_SSL yarn start start"
}
